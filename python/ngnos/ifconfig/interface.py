...
from ngnos.validate import assert_boolean
from ngnos.validate import assert_list
from ngnos.validate import assert_mac
from ngnos.validate import assert_mtu
from ngnos.validate import assert_positive
from ngnos.validate import assert_range
...
class Interface(Control):
    ...
    def __init__(self, ifname, **kargs):
        ...
        ngnos@ngnos# touch /tmp/ngnos.ifconfig.debug
    ...
    def _cleanup_mss_rules(self, table, ifname):
        commands = []
        results = self._cmd(f'nft -a list chain {table} NGNOS_TCP_MSS').split("\n")
        for line in results:
            if f'oifname "{ifname}"' in line:
                handle_search = re.search('handle (\d+)', line)
                if handle_search:
                    self._cmd(f'nft delete rule {table} NGNOS_TCP_MSS handle {handle_search[1]}')
    ...
    def set_tcp_ipv4_mss(self, mss):
        ...
        nft_prefix = 'nft add rule raw NGNOS_TCP_MSS'
    ...
    def set_tcp_ipv6_mss(self, mss):
        ...
        nft_prefix = 'nft add rule ip6 raw NGNOS_TCP_MSS'
        base_cmd = f'oifname "{self.ifname}" tcp flags & (syn|rst) == syn'
        if mss == 'clamp-mss-to-pmtu':
            self._cmd(f"{nft_prefix} '{base_cmd} tcp option maxseg size set rt mtu'")
        elif int(mss) > 0:
    ...
    def set_alias(self, ifalias=''):
        ...
        Example:
        >>> from ngnos.ifconfig import Interface
        >>> Interface('eth0').set_alias('ngNOS upstream interface')
    ...
    def set_dhcp(self, enable):
        ...
        if enable and 'disable' not in self._config:
            if dict_search('dhcp_options.host_name', self._config) == None:
                ...
                # maybe change to ngnos hostd client ???
                hostname = 'ngnos'
    ...
    def set_xdp(self, state):
        ...
        # https://ngnos.dev/T3448 - there is (yet) no RPI support for XDP
        ...
        if state:
            ...
            cmd = f'xdp_loader -d {ifname} --auto-mode -F --progsec xdp_router ' \
                  f'--filename /usr/share/ngnos/xdp/xdp_prog_kern.o && ' \
