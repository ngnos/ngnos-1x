...
SOCKET_PATH = "ipc:///run/ngnos-hostsd/ngnos-hostsd.sock"

class ngNOSHostsdError(Exception):
    pass
...
class Client(object):
    def __init__(self):
        try:
            ...
            raise ngNOSHostsdError("Could not connect to ngnos-hostsd")
    ...
    def _communicate(self, msg):
        try:
            ...
            if 'error' in reply:
                raise ngNOSHostsdError(reply['error'])
            else:
                return reply["data"]
        except zmq.error.Again:
            raise ngNOSHostsdError("Could not connect to ngnos-hostsd")
