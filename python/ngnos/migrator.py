...
log_file = os.path.join(ngnos.defaults.directories['config'], 'ngnos-migrate.log')

class MigratorError(Exception):
    pass

class Migrator(object):
    def __init__(self, config_file, force=False, set_vintage='ngnos'):
    ...
    def read_config_file_versions(self):
        ...
        cfg_versions = component_version.from_file(cfg_file, vintage='ngnos')

        if cfg_versions:
            self._config_file_vintage = 'ngnos'
            component_versions = cfg_versions
    ...
    def update_vintage(self):
        ...
        if self._config_file_vintage not in ['vyatta', 'ngnos']:
    ...
    def run_migration_scripts(self, config_file_versions, system_versions):
        ...
        for key in sys_keys:
            ...
            migrate_script_dir = os.path.join(
                    ngnos.defaults.directories['migrate'], key)
        ...
        del os.environ['NGNOS_MIGRATION']
        return rev_versions
    ...
    def write_config_file_versions(self, cfg_versions):
        ...
        if self._config_file_vintage == 'ngnos':
            component_version.write_version_footer(cfg_versions,
                                                   self._config_file,
                                                   vintage='ngnos')
    ...
    def save_json_record(self, component_versions: dict):
        ...
        """
        mask = os.umask(0o113)
        version_file = ngnos.defaults.component_version_json
        try:
    ...
    def run(self):
        ...
        Update vintage ('vyatta' or 'ngnos'), if needed.
