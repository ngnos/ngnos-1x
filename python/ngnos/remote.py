...
from ngnos.util import begin
from ngnos.util import cmd
from ngnos.util import make_incremental_progressbar
from ngnos.util import make_progressbar
from ngnos.util import print_error
...
class HttpC:
    ...
    def _establish(self):
        ...
        session.headers.update({'User-Agent': 'ngNOS/' + get_version()})
        if self.username:
        ...
        with self._establish() as s:
            ...
            with s.get(final_urlstring, stream=True) as r, open(location, 'wb') as f:
                if self.progressbar and size:
                    ...
                    #  files (like entire ngNOS images) don't occupy much memory.
