...
"""
A library for reading ngNOS running config data.

This library is used internally by all config scripts of ngNOS,
but its API should be considered stable and safe to use
in user scripts.

Note that this module will not work outside ngNOS.

Node taxonomy
#############

There are multiple types of config tree nodes in ngNOS, each requires
...
ngNOS has two distinct modes: operational mode and configuration mode. When a user logins,
...
from ngnos.configsource import ConfigSource, ConfigSourceSession
...
class Config(object):
    ...
    def _make_path(self, path):
        ...
        # libngnosconfig paths are lists, but since node names cannot contain whitespace,
    ...
    def exists(self, path):
        ...
        if self._session_config.exists(self._make_path(path)):
            ...
            # libngnosconfig exists() works only for _nodes_, not _values_
            ...
            except ngnos.configtree.ConfigTreeError:
    ...
    def get_config_dict(self, path=[], effective=False, key_mangling=None,
        ...
        conf_dict = ngnos.util.get_sub_dict(root_dict, lpath, get_first_key)
        ...
        if not key_mangling:
            conf_dict = ngnos.xml.multi_to_list(xmlpath, conf_dict)
            return conf_dict
        ...
        conf_dict = ngnos.util.mangle_dict_keys(conf_dict, key_mangling[0], key_mangling[1], abs_path=xmlpath, no_tag_node_value_mangle=no_tag_node_value_mangle)
