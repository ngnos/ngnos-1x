...
from ngnos.xml import component_version
from ngnos.version import get_version
from ngnos.defaults import directories

DEFAULT_CONFIG_PATH = os.path.join(directories['config'], 'config.boot')

def from_string(string_line, vintage='ngnos'):
    """
    ...
    if vintage == 'ngnos':
        if re.match(r'// ngnos-config-version:.+', string_line):
            if not re.match(r'// ngnos-config-version:\s+"([\w,-]+@\d+:)+([\w,-]+@\d+)"\s*', string_line):
...
def from_file(config_file_name=DEFAULT_CONFIG_PATH, vintage='ngnos'):
    """
...
def version_footer(ver: dict, vintage='ngnos') -> str:
    """
    ...
    if vintage == 'ngnos':
        ret_str = (f'// Warning: Do not remove the following line.\n'
                +  f'// ngnos-config-version: "{ver_str}"\n'
...
def system_footer(vintage='ngnos') -> str:
    """
...
def write_version_footer(ver: dict, file_name, vintage='ngnos'):
    """
...
def write_system_footer(file_name, vintage='ngnos'):
    """
...
def remove_footer(file_name):
    ...
    for line in fileinput.input(file_name, inplace=True):
        ...
        if re.match(r'/\* Release version:.+ \*/$', line):
            continue
        if re.match('// ngnos-config-version:.+', line):
            continue
