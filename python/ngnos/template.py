...
def nft_action(ngnos_action):
    if ngnos_action == 'accept':
        return 'return'
    return ngnos_action

@register_filter('nft_rule')
def nft_rule(rule_conf, fw_name, rule_id, ip_name='ip'):
    from ngnos.firewall import parse_rule
...
def nat_rule(rule_conf, rule_id, nat_type, ipv6=False):
    from ngnos.nat import parse_nat_rule
...
def nat_static_rule(rule_conf, rule_id, nat_type):
    from ngnos.nat import parse_nat_static_rule
...
def range_to_regex(num_range):
    """Convert range of numbers or list of ranges
       ...
       '(1[1-2]|1[4-5])'
    """
    from ngnos.range_regex import range_to_regex
...
@register_test('ngnos_defined')
def ngnos_defined(value, test_value=None, var_type=None):
    """
    Jinja2 plugin to test if a variable is defined and not none - ngnos_defined
