...
"""
A library for retrieving value dicts from ngNOS configs in a declarative fashion.
"""
import os
import json
...
def retrieve_config(path_hash, base_path, config):
    """
    Retrieves a ngNOS config as a dict according to a declarative description
    ...
    Args:
        ...
        config (ngnos.config.Config): A ngNOS config object
...
def is_node_changed(conf, path):
   from ngnos.configdiff import get_config_diff
...
def get_interface_dict(config, base, ifname=''):
    ...
    if not ifname:
        ...
        if 'NGNOS_TAGNODE_VALUE' not in os.environ:
            raise ConfigError('Interface (NGNOS_TAGNODE_VALUE) not specified')
        ifname = os.environ['NGNOS_TAGNODE_VALUE']
