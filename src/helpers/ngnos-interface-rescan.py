...
from ngnos.util import get_cfg_group_id

debug = False

ngnos_udev_dir = directories['ngnos_udev_dir']
ngnos_log_dir = directories['log']
log_file = os.path.splitext(os.path.basename(__file__))[0]
ngnos_log_file = os.path.join(ngnos_log_dir, log_file)

logger = logging.getLogger(__name__)
handler = logging.FileHandler(ngnos_log_file, mode='a')
...
def get_new_interfaces() -> dict:
    """ Read any new interface data left in /run/udev/ngnos by ngnos_net_name
    """
    interfaces = {}

    for intf in os.listdir(ngnos_udev_dir):
        path = os.path.join(ngnos_udev_dir, intf)
        try:
            with open(path) as f:
                hwid = f.read().rstrip()
        except OSError as e:
...
def main():
    ...
    # logging config file
    os.chown(ngnos_log_file, 0, get_cfg_group_id())
    os.chmod(ngnos_log_file,
