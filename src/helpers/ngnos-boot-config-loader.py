...
from ngnos.defaults import directories, config_status
...
LOG_FILE = LOG_DIR + '/ngnos-boot-config-loader.log'

try:
    with open('/proc/cmdline', 'r') as f:
        cmdline = f.read()
    if 'ngnos-debug' in cmdline:
        os.environ['NGNOS_DEBUG'] = 'yes'
    if 'ngnos-config-debug' in cmdline:
        os.environ['NGNOS_DEBUG'] = 'yes'
        trace_config = True
...
def failsafe(config_file_name):
    ...
    if not config.exists(['system', 'login', 'user', 'ngnos',
                          'authentication', 'encrypted-password']):
        print("No password entry in default config file;")
        print("unable to recover password for user 'ngnos'.")
        sys.exit(1)
    else:
        passwd = config.return_value(['system', 'login', 'user', 'ngnos',
                                      'authentication',
                                      'encrypted-password'])

    cmd(f"useradd -s /bin/bash -G 'users,sudo' -m -N -p '{passwd}' ngnos")
...
if __name__ == '__main__':
    ...
    session = ConfigSession(os.getpid(), 'ngnos-boot-config-loader')
